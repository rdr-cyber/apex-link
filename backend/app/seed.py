...
# Search evidence texts for entity identifiers so CaseEntity records
# carry source_evidence_id. This is critical: generate_relationships()
# groups entities by source_evidence_id to find co-occurrences.
# Without this, zero relationships are generated by the pipeline.
def _find_evidence_for_entity(entity_idx: int, evidence_texts: list[str]) -> list[int]:
    """Return indices of evidence texts that mention this entity's identifiers."""
    if entity_idx >= len(DEMO_ENTITIES):
        return []
    ent = DEMO_ENTITIES[entity_idx]
    hits = []
    # Search for display value, canonical value, and normalized forms
    search_terms = set()
    for field in (ent["display"], ent["canonical"], ent["normalized"]):
        if field:
            search_terms.add(field.lower())
    # For phones, also search without country code/spaces
    if ent["entity_type"] == "PHONE":
        digits = ''.join(c for c in ent["display"] if c.isdigit())
        if digits:
            search_terms.add(digits)
    # For persons, search first and last name separately
    if ent["entity_type"] == "PERSON":
        for word in ent["canonical"].split():
            search_terms.add(word.lower())
    # For vehicles, search with and without spaces
    if ent["entity_type"] == "VEHICLE":
        search_terms.add(ent["display"].replace(" ", "").lower())
        search_terms.add(ent["canonical"].lower())

    for idx, text in enumerate(evidence_texts):
        text_lower = text.lower()
        if any(term in text_lower for term in search_terms if len(term) >= 3):
            hits.append(idx)
    return hits


# Pre-compute entity-to-evidence mapping
ENTITY_EVIDENCE_MAP: dict[int, list[int]] = {}
for _ei in range(len(DEMO_ENTITIES)):
    ENTITY_EVIDENCE_MAP[_ei] = _find_evidence_for_entity(_ei, EVIDENCE_TEXTS)
...
async def seed():
    ...
    async with async_session_factory() as db:
        ...
        # Create CaseEntity links — with source_evidence_id
        # This is critical: generate_relationships() groups entities by
        # source_evidence_id to discover co-occurring entities.
        ce_count = 0
        for entity_idx, case_indices in ENTITY_CASE_MAP.items():
            if entity_idx >= len(entities):
                continue
            entity = entities[entity_idx]
            # Find which evidence texts mention this entity
            ev_indices = ENTITY_EVIDENCE_MAP.get(entity_idx, [])
            for case_idx in case_indices:
                if case_idx >= len(cases):
                    continue
                # Pick the first evidence in this case that mentions this entity
                source_ev_id = None
                for ev_i in ev_indices:
                    if ev_i < len(evidence_list) and evidence_list[ev_i].case_id == cases[case_idx].id:
                        source_ev_id = evidence_list[ev_i].id
                        break
                # Fallback: use any evidence from this case
                if source_ev_id is None:
                    for ev in evidence_list:
                        if ev.case_id == cases[case_idx].id:
                            source_ev_id = ev.id
                            break
                ce = CaseEntity(
                ...
                ce_count += 1
        await db.flush()
        print(f"  Created {ce_count} case-entity links (with source evidence)")
        ...
        rel_count = 0
        relationship_defs = [
            # ── Cluster A: Vikram Patel's network ──
            ...
            # Co-occurrence in evidence 0-1
            (1, 5, "MENTIONED_WITH", "Phones 9876543210 and 8765432109 co-occur in call records"),
            (1, 2, "MENTIONED_WITH", "Phone 9876543210 and email vikram.p co-occur"),

            # ── Bridge A↔B: phone 7654321098 + UPI sunil.m ──
            (10, 0, "MENTIONED_WITH", "Bridge phone 7654321098 linked to Vikram via phishing"),
            (10, 3, "MENTIONED_WITH", "Bridge phone 7654321098 co-occurs with Vikram UPI"),
            (11, 3, "MENTIONED_WITH", "Bridge UPI sunil.m linked to Vikram UPI in fund transfers"),

            # ── Cluster B: Meera Joshi's network ──
            ...
            # Co-occurrence in evidence 2-3
            (13, 14, "MENTIONED_WITH", "Phone 6543210987 and email meera.j co-occur"),
            (13, 15, "MENTIONED_WITH", "Phone 6543210987 and HDFC account co-occur in transactions"),
            (13, 16, "MENTIONED_WITH", "Phone 6543210987 and IP 10.0.0.55 co-occur"),
            (14, 15, "MENTIONED_WITH", "Email meera.j and HDFC account co-occur"),

            # ── Bridge B↔A: connect Meera cluster to bridge entities ──
            (12, 11, "MENTIONED_WITH", "Meera linked to bridge UPI sunil.m via fund transfers"),
            (13, 10, "MENTIONED_WITH", "Phone 6543210987 co-occurs with bridge phone 7654321098"),

            # ── Suresh Nair's network ──
            ...
            # Co-occurrence in evidence 3
            (17, 13, "MENTIONED_WITH", "Phones 5432109876 and 6543210987 co-occur in Bandra transactions"),
            (17, 19, "MENTIONED_WITH", "Phone 5432109876 and email suresh.n co-occur"),
            (20, 13, "MENTIONED_WITH", "QuickTrade Solutions linked to phone 6543210987"),
            (20, 17, "MENTIONED_WITH", "QuickTrade Solutions linked to phone 5432109876"),
            (20, 19, "MENTIONED_WITH", "QuickTrade Solutions linked to suresh.n email"),
            ...
            # Co-occurrence
            (23, 24, "MENTIONED_WITH", "Phone 4321098765 and email ravi.g co-occur"),
            (23, 27, "MENTIONED_WITH", "Phones 4321098765 and 3210987654 co-occur in Delhi"),

            # ── Deepa Menon's network ──
            ...
            # Co-occurrence
            (27, 29, "MENTIONED_WITH", "Phone 3210987654 and email deepa.m co-occur"),
            (27, 30, "MENTIONED_WITH", "Phone 3210987654 and Metro Logistics co-occur"),
            (29, 30, "MENTIONED_WITH", "Email deepa.m and Metro Logistics co-occur"),

            # ── Bridge B↔C: QuickTrade connects to Ravi/Deepa cluster ──
            (20, 23, "MENTIONED_WITH", "QuickTrade Solutions linked to phone 4321098765"),
            (20, 27, "MENTIONED_WITH", "QuickTrade Solutions linked to phone 3210987654"),
            (20, 22, "MENTIONED_WITH", "QuickTrade Solutions linked to Ravi Gupta"),
            (20, 30, "MENTIONED_WITH", "QuickTrade Solutions and Metro Logistics co-occur"),
            (31, 16, "MENTIONED_WITH", "IPs 172.16.0.99 and 10.0.0.55 both active in fraud"),
